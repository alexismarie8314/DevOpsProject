...
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://postgres:root@localhost:5432/flask_dev"
db=SQLAlchemy(app)
...
def update(id):
    ...
    if request.method=='POST':
        ...
        if uto.email!=request.form.get('email'):
            flash('Email cannot be changed. A valid email adress canot be changed to another one. ',category='error') 
        elif len(email)<3:
            flash('_> Email > 4 charac',category='error')
        elif len(username)<2:
            flash('_> Username > 4 charac',category='error')
